...
check_environment() {
    ...
    # 检查是否在项目根目录 - 使用更合适的文件作为标识
    local project_files=("llm_proxy.py" "config_manager.py" "requirements.txt")
...
# 启动服务 - 使用正确的Python文件
python llm_proxy.py >> "$LOG_FILE" 2>&1 &
PID=$!
...
EOF
    ...
    if curl -s http://localhost:8080/ > /dev/null 2>&1; then
        ...
        print_info "查看日志命令: tail -f ~/.llm-proxy/logs/llm-proxy.log"
    fi
